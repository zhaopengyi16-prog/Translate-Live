...
{
    // Reasoning/thinking for real-time translation is disabled when the provider supports it.
    ...
    public class BaseLLMRequestData(string model, List<BaseLLMConfig.Message> messages, double temperature)
    {
        ...
        // Keep enough room for a short translation and one bounded retry.
        public int max_tokens { get; set; } = 512;
        public bool stream { get; set; } = false;
    }

    public class DeepSeekLLMRequestData
        : BaseLLMRequestData
    {
        public DeepSeekLLMRequestData(
            string model,
            List<BaseLLMConfig.Message> messages,
            double temperature)
            : base(model, messages, temperature)
        {
            // Real-time translation benefits from receiving the first usable
            // content token instead of waiting for the complete JSON body.
            stream = true;
            max_tokens = 192;
        }
