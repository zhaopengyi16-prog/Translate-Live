...
namespace LiveCaptionsTranslator.utils
{
    public static class TextUtil
    {
        ...
        /// <summary>
        /// Normalizes visual line wrapping from Windows Live Captions without
        /// inventing punctuation. A line break is a layout detail, not a speech
        /// boundary, so Latin text receives a space while adjacent CJK text does
        /// not.
        /// </summary>
        public static string NormalizeCaptionWhitespace(string? text)
        {
        ...
        private static bool ShouldInsertCaptionSpace(char previous, char current)
        {
            if (isCJChar(previous) && isCJChar(current))
                return false;

            const string closingPunctuation = ",.;:!?)]}，。；：！？、）】》」』…";
            const string openingPunctuation = "([{（【《「『";
            if (closingPunctuation.Contains(current) || openingPunctuation.Contains(previous))
                return false;
